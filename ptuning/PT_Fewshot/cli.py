...
def main():
    ...
    parser.add_argument("--unlabel_file", default='', type=str, help="data file for pattern extraction")
    parser.add_argument("--rel2id_file", default=None, type=str, help="")
    ...
    pdb.set_trace()
    rel2id = None
    if args.rel2id_file is not None:
        ref = open(args.rel2id_file, 'r', encoding='utf-8')
        rel2id = json.load(ref)
    if args.task_name == 'multire' or args.task_name == 'multicon' or args.task_name == 'sinre':
        processor = PROCESSORS[args.task_name](rel2id)
    else:
    ...
    train_ex, eval_ex, dev32_ex = args.train_examples, args.eval_examples, args.dev32_examples#-1, -1, -1
    ...
    #把数据按照example对象的格式存储起来
    train_data = load_examples(
        args.task_name, args.data_dir, TRAIN_SET, num_examples=train_ex, num_examples_per_label=train_ex_per_label, rel2id=rel2id)

    eval_data = load_examples(
        args.task_name, args.data_dir, eval_set, num_examples=eval_ex, num_examples_per_label=eval_ex_per_label, rel2id=rel2id)

    dev_data = load_examples(
        args.task_name, args.data_dir, DEV_SET, num_examples=dev32_ex, num_examples_per_label=dev32_ex_per_label, rel2id=rel2id)
