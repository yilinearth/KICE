...
class Preprocessor(ABC):
    ...
    def __init__(self, wrapper, task_name, pattern_id: int = 0, rel2id=None):
        """
        ...
        self.pvp = PVPS[task_name](self.wrapper, pattern_id, rel2id=rel2id)  # type: PVP
...
class MLMPreprocessor(Preprocessor):
    ...
    def get_input_features(self, example: InputExample, labelled: bool, priming: bool = False, type=None,
                           **kwargs) -> InputFeatures:

        input_ids, token_type_ids, block_flag = self.pvp.encode(example, type=type)
        ...
        attention_mask = attention_mask + ([0] * padding_length)#哪个位置不是pad
        ...
        if labelled:
            mlm_labels = self.pvp.get_mask_positions(input_ids)#哪个位置是mask
        else:
