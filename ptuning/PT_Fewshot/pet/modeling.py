...
def init_model(config: WrapperConfig, rel2id=None) -> TransformerModelWrapper:
    ...
    model = TransformerModelWrapper(config, rel2id)
    return model
...
def train_pet(train_data: List[InputExample],
    ...
    for pattern_id in pattern_ids:
        for iteration in range(repetitions):
            ...
            ###
            if os.path.exists(pattern_iter_output_dir) and do_train:
            ...
            if do_eval:
                ...
                # if not wrapper:
                wrapper = TransformerModelWrapper.from_pretrained(pattern_iter_output_dir, rel2id)

                eval_result = evaluate(wrapper, eval_data, eval_config)
                dev32_result = evaluate(wrapper, dev_data, eval_config)
    ...
    if do_eval:
        logger.info("=== OVERALL RESULTS ===")#results是eval result
...
def train_single_model(train_data: List[InputExample],
    ...
    model = TransformerModelWrapper.from_pretrained(pattern_iter_output_dir, rel2id)
...
def evaluate(model: TransformerModelWrapper,
    ...
    results = model.eval(eval_data=eval_data,
                         ...
                         n_gpu=config.n_gpu)
    predictions = np.argmax(results['logits'], axis=1)#[eval_data_size]
    scores = {}
    for metric in metrics:
        if metric == 'acc':
            ...
            eval_result =  get_nero_eval(results['labels'], predictions, model.rel2id)
...
def get_nero_eval(gold_label, pred_label, rel2id):
...
def evaluate_mask_word(model: TransformerModelWrapper,
             eval_data: List[InputExample],
             config: EvalConfig):
    results = model.eval_words(eval_data=eval_data,
                         per_gpu_eval_batch_size=config.per_gpu_eval_batch_size,
                         n_gpu=config.n_gpu)
    return results
