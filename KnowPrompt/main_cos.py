...
import lit_models
from lit_models import cos_transformer
import yaml
import time
...
def _setup_parser():
    ...
    #cos arguments
    parser.add_argument("--old_ckpt", type=str, default=None)
    parser.add_argument('--self_training_eps', type=float, default=0.5, help='threshold for confidence')
    parser.add_argument('--self_training_confreg', type=float, default=0.05,help='confidence smooth power')
    parser.add_argument('--self_training_power', type=float, default=2,help='power of pred score')
    parser.add_argument('--calc_loss_weight', type=float, default=0.5,
    ...
    litmodel_class = _import_class(f"lit_models.cos_transformer.{temp_args.litmodel_class}")
...
def main():
    ...
    litmodel_class = _import_class(f"lit_models.cos_transformer.{args.litmodel_class}")
    ...
    tea_lit_model = litmodel_class(args=args, model=model, tokenizer=data.tokenizer)
    model_state = torch.load(args.old_ckpt)["state_dict"]
    for key in list(model_state.keys()):
        if 'old_model' in key:
            del(model_state[key])

    tea_lit_model.load_state_dict(model_state)
    tea_lit_model = tea_lit_model.cuda()

    lit_model = litmodel_class(args=args, model=model, tokenizer=data.tokenizer,
                               old_model=tea_lit_model, self_training_eps=args.self_training_eps, self_training_confreg=args.self_training_confreg,
                               self_training_power=args.self_training_power, calc_loss_weight=args.calc_loss_weight)
    ...
    model_checkpoint = pl.callbacks.ModelCheckpoint(monitor="Eval/f1", mode="max",
        filename='step{}/model'.format(args.step),
        dirpath="output",
