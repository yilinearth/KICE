...
class wiki80Processor(DataProcessor):
    ...
    def __init__(self, data_path, use_prompt, dataset='tacred'):
        super().__init__()
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join(data_dir, "RE_model_data/train.json")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join('../data/step0', "RE_model_data/dev.json")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join('../data', self.dataset, "my_test.json")), "test")

    def get_can_examples(self, data_dir):
        """See base class."""
        if data_dir[-1] == '0':
            return self._create_examples(
                self._read_json(os.path.join(data_dir, "rt_aft_samp_valid_emb.json")), "pred")
        else:
            return self._create_examples(
                self._read_json(os.path.join(data_dir, "rt_aft_anno.json")), "pred")

    def get_unlabel_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join(data_dir, "rt_aft_pred.json")), "pred")

    def get_confuse_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join(data_dir, "active/confuse_rule.json")), "pred")

    def get_boot_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join(data_dir, "rule_data/boot_rules.json")), "pred")

    def get_my_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join(data_dir, "test.txt")), "pred")

    def get_labels(self, negative_label="no_relation"):
        data_dir = self.data_dir
        """See base class."""
        with open(os.path.join('../data', self.dataset, 'my_rel2id.json'), "r", encoding='utf-8') as reader:
...
def get_dataset(mode, args, tokenizer, processor):
    ...
    if mode == "train":
        ...
        examples = processor.get_can_examples(args.data_dir)
    elif mode == "unlabel":
        examples = processor.get_unlabel_examples(args.data_dir)
    elif mode == "confuse":
        examples = processor.get_confuse_examples(args.new_data_dir)
    elif mode == "boot":
        examples = processor.get_boot_examples(args.data_dir)
    elif mode == "my_test":
        examples = processor.get_my_test_examples(args.data_dir)
    else:
