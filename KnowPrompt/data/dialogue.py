...
class WIKI80(BaseDataModule):
    def __init__(self, args, model=None) -> None:
        super().__init__(args)
        self.processor = processors[self.args.task_name](self.args.data_dir, self.args.use_prompt, dataset=args.dataset)
        self.tokenizer = AutoTokenizer.from_pretrained(self.args.model_name_or_path)
...
class WIKI80_pred(BaseDataModule):
    def __init__(self, args, model=None, type='can') -> None:
        super().__init__(args)
        self.processor = processors[self.args.task_name](self.args.data_dir, self.args.use_prompt, dataset=args.dataset)
        self.tokenizer = AutoTokenizer.from_pretrained(self.args.model_name_or_path)
    ...
    def setup(self, stage=None):
        if self.type == 'can':
            self.data_pred = get_dataset("can", self.args, self.tokenizer, self.processor)
        elif self.type == 'my_test':
            self.data_my_test = get_dataset("my_test", self.args, self.tokenizer, self.processor)
        else:
            self.data_unlabel = get_dataset("unlabel", self.args, self.tokenizer, self.processor)
            self.data_confuse = get_dataset("confuse", self.args, self.tokenizer, self.processor)
            self.data_boot = get_dataset("boot", self.args, self.tokenizer, self.processor)
