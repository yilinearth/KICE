...
class BertLitModel(BaseLitModel):
    ...
    """
    def __init__(self, model, args, tokenizer, old_model=None, self_training_eps=0.1, self_training_confreg=0.05, self_training_power=0.1, calc_loss_weight=0.1):
        super().__init__(model, args)
        self.tokenizer = tokenizer

        with open(f"../data/{args.dataset}/my_rel2id.json", "r") as file:
            rel2id = json.load(file)
        
        Na_num = 0
        for k, v in rel2id.items():
            if k == "NA" or k == "no relation" or k == "Other":
                Na_num = v
                break
        ...
        self.self_training_loss = nn.KLDivLoss(reduction = 'none')
        self.self_training_eps = self_training_eps
        self.self_training_confreg = self_training_confreg
        self.self_training_power = self_training_power
        self.calc_loss_weight = calc_loss_weight
    ...
    def _init_label_word(self, ):
        ...
        label_path = f"../data/{dataset_name}/{model_name_or_path}_{dataset_name}.pt"
    ...
    def training_step(self, batch, batch_idx):  # pylint: disable=unused-argument
        ...
        logits = result.logits #[batch size, max_length, verbal size]

        pseudo_result = self.old_model(input_ids, attention_mask, return_dict=True, output_hidden_states=True)
        pseudo_logits = pseudo_result.logits
        pseudo_logits = self.pvp(pseudo_logits, input_ids)
        pseudo_loss = self.calc_loss_weight * self.calc_loss(input=torch.log(torch.softmax(logits, -1)),
                                        target=pseudo_logits,
                                        loss=self.self_training_loss,
                                        thresh=self.self_training_eps,
                                        soft=True,
                                        conf='entropy',
                                        confreg=self.self_training_confreg)

        output_embedding = result.hidden_states[-1]
        logits = self.pvp(logits, input_ids) #[batch_size, class_num]
        ...
        loss = self.loss_fn(logits, labels) + self.t_lambda * ke_loss + pseudo_loss
    ...
    def calc_loss(self, input, target, loss, thresh=0.95, soft=True, conf='max', confreg=0.1):
        ...
        if conf == 'max':
            ...
            w = torch.FloatTensor([1 if x == True else 0 for x in weight > thresh]).cuda()
    ...
    def soft_frequency(self, logits,  probs=False, soft = True):
        ...
        """
        power = self.self_training_power
        if not probs:
            softmax = nn.Softmax(dim=1)
    ...
    def test_epoch_end(self, outputs) -> None:
        ...
        self.log("Test/f1", f1)
        self.log("test micro_f1", mic_f1)

    def get_pred_logits(self, data_loader):
        self.model.eval()
        outputs = []
        total_data = 0
        with torch.no_grad():
            for batch in data_loader:
                ...
                total_data += len(logits)
                print(total_data)
                outputs.append({"test_logits": logits.detach().cpu().numpy(), "test_labels": labels.detach().cpu().numpy()})
        logits = np.concatenate([o["test_logits"] for o in outputs])
        labels = np.concatenate([o["test_labels"] for o in outputs])
        return {"test_logits": logits, "test_labels": labels}

    def get_my_test_result(self, data_loader):
        self.model.eval()
        outputs = []
        total_data = 0
        with torch.no_grad():
            for batch in data_loader:
                ...
                print(total_data)
                outputs.append(
                    {"test_logits": logits.detach().cpu().numpy(), "test_labels": labels.detach().cpu().numpy()})
        logits = np.concatenate([o["test_logits"] for o in outputs])
        labels = np.concatenate([o["test_labels"] for o in outputs])
        f1 = self.eval_fn(logits, labels)['f1']
        return f1
