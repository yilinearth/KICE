...
from .util import f1_eval, compute_f1, acc, f1_score, wrench_f1_score
...
class BertLitModel(BaseLitModel):
    ...
    def __init__(self, model, args, tokenizer, old_model=None, self_training_eps=0.1, self_training_confreg=0.05, self_training_power=0.1, calc_loss_weight=0.1):
        ...
        self.eval_fn = f1_eval if "dialogue" in args.data_dir else partial(wrench_f1_score, rel_num=num_relation, na_num=Na_num)
        self.best_f1 = 0
    ...
    def training_step(self, batch, batch_idx):  # pylint: disable=unused-argument
        ...
        with torch.no_grad():
            pseudo_result = self.old_model.model(input_ids, attention_mask, return_dict=True, output_hidden_states=True)
            pseudo_logits = pseudo_result.logits
            pseudo_logits = self.old_model.pvp(pseudo_logits, input_ids)
...
class GPTLitModel(BaseLitModel):
    ...
    def validation_epoch_end(self, outputs) -> None:
        ...
        f1 = wrench_f1_score(logits, labels)
        self.log("Eval/f1", f1)
        if f1 > self.best_f1:
            self.best_f1 = f1
        self.log("Eval/best_f1", self.best_f1, prog_bar=True, on_epoch=True)
