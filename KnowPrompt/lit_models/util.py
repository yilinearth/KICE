...
def wrench_f1_score(output, label, rel_num=42, na_num=13):
    if output.shape != label.shape:
        output = np.argmax(output, axis=-1)

    mic_f1 = cls_metric.f1_score(label, output, average='micro')
    return dict(f1=mic_f1)


# def f1_score(output, label, rel_num=42, na_num=13, val_threshold = 2.7):
#     correct_by_relation = Counter()
#     guess_by_relation = Counter()
#     gold_by_relation = Counter()
#     output = torch.FloatTensor(output)
#     for i in range(len(output)):
#         output[i] = torch.FloatTensor(output[i])
#         multi_score, guess = torch.max(output[i], 0)
#         val = torch.sum(output[i] * -torch.log(output[i]))
