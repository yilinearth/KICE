...
class SentenceREDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if item['relation'] == 'no relation' and item['relation'] not in self.rel2id:
            return [0] + seq

        return [self.rel2id[item['relation']], index] + seq # label, seq1, seq2, ...
    ...
    def collate_fn(data):
        ...
        return [batch_labels, index] + batch_seqs
...
class BagREDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        seqs = None
        if self.data[bag[0]]['relation'] not in self.rel2id:
            rel = 0
        else:
