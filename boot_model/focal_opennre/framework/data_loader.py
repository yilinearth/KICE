...
class SentenceREDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # soft_label = torch.FloatTensor([1.0 / 40] * 40)
        if 'relid2weight' in item:
            soft_label = torch.FloatTensor(item['relid2weight'])
        else:
            soft_label = torch.zeros(40)
            soft_label[self.rel2id[item['relation']]] = 1.0

        return [soft_label] + [self.rel2id[item['relation']]] + seq # label, seq1, seq2, ...
    ...
    def collate_fn(data):
        ...
        seqs = data[2:]
        ######
        batch_soft_labels = torch.Tensor([item.detach().numpy() for item in soft_labels])
        batch_hard_labels = torch.tensor(hard_labels).long() # (B)
        batch_seqs = []
        for seq in seqs:
            batch_seqs.append(torch.cat(seq, 0)) # (B, L)
        return [batch_soft_labels] + [batch_hard_labels] + batch_seqs
...
class BagREDataset(data.Dataset):
    ...
    def __init__(self, path, rel2id, tokenizer, entpair_as_bag=False, bag_size=0, mode=None):
        ...
        if mode == None:
            ...
            for idx, item in enumerate(self.data):
                if 'id' not in item['h']:
                    item['h']['id'] = item['h']['name']
                    item['t']['id'] = item['t']['name']
                # Annotated test set
    ...
    def eval(self, pred_result, threshold=0.5):
        ...
        return {'prec': prec, 'rec': rec, 'max_micro_f1': max_micro_f1, 'max_macro_f1': max_macro_f1, 'auc': auc, 'p@100': np_prec[99], 'p@200': np_prec[199], 'p@300': np_prec[299], 'r@100': np_rec[99], 'r@200': np_rec[199], 'r@300': np_rec[299], 'avg_p300': (np_prec[99] + np_prec[199] + np_prec[299]) / 3, 'micro_f1': micro_f1, 'micro_p': micro_p, 'micro_r': micro_r, 'macro_f1': macro_f1, 'macro_p': macro_p, 'macro_r': macro_r, 'max_micro_f1_each_relation': max_micro_f1_each_relation}


    def my_eval(self, pred_result, threshold=0.5):
        ...
        for i, item in enumerate(sorted_pred_result):
            pred_rel = item['pred_rel']
            rel2pred[pred_rel].append(item['score'])
            # TP
            if (item['entpair'][0], item['entpair'][1], item['pred_rel']) in self.facts:
                rel2pr[pred_rel][0] += 1
                rel2y[pred_rel].append(1)
            # 负类但预测成了正类（FP）
            else:
                rel2y[pred_rel].append(0)
                rel2pr[pred_rel][1] += 1

            for rel in self.rel2id:
                #预测为负类
                if rel != pred_rel:
                    rel2pred[rel].append(item['logits'][self.rel2id[rel]])
                    #正例预测为负类(FN)
                    if (item['entpair'][0], item['entpair'][1], rel) in self.facts:
                        rel2y[rel].append(1)
                        rel2pr[rel][3] += 1
                    #负类预测为负类
                    else:
            ...
            total_prec, total_rec, total_f1, total_auc = 0.0, 0.0, 0.0, 0.0
            valid_rel_count = 0
            for rel in self.rel2id:
                if rel2pr[rel][0]+rel2pr[rel][1]+rel2pr[rel][3] != 0:
                    valid_rel_count += 1

                rel_pre = 0 if (rel2pr[rel][0]+rel2pr[rel][1]) == 0 else rel2pr[rel][0] / (rel2pr[rel][0]+rel2pr[rel][1])
                total_prec += rel_pre

                rel_re = 0 if (rel2pr[rel][0]+rel2pr[rel][3]) == 0 else rel2pr[rel][0] / (rel2pr[rel][0]+rel2pr[rel][3])
                total_rec += rel_re

                rel_f1 = 0 if (rel_pre + rel_re) == 0 else (2*rel_pre*rel_re) / (rel_pre + rel_re)
                total_f1 += rel_f1
                rel2f1[rel] = rel_f1
                if (sum(rel2y[rel]) == 0 or sum(rel2y[rel]) == len(rel2y[rel])):
                    sin_auc = 0
                else:
                    sin_auc = roc_auc_score(np.array(rel2y[rel]), np.array(rel2pred[rel]))
                total_auc += sin_auc

            total_prec = total_prec / valid_rel_count
            total_rec = total_rec / valid_rel_count
            prec.append(total_prec)
            rec.append(total_rec)
            sin_micro_f1 = 0 if (total_prec + total_rec) == 0 else (2 * total_prec * total_rec) / (total_prec + total_rec)
            micro_f1.append(sin_micro_f1)
            macro_f1.append(total_f1 / valid_rel_count)
            auc.append(total_auc / valid_rel_count)

        return {'prec': prec, 'rec': rec, 'auc': auc[-1], 'p@100': prec[99], 'p@200': prec[199], 'p@300': prec[299], 'r@100': rec[99],
                'r@200': rec[199], 'r@300': rec[299], 'rel2f1': rel2f1,
                'micro_f1@100': micro_f1[99], 'micro_f1@200': micro_f1[199], 'micro_f1@300': micro_f1[299], 'micro_f1': micro_f1[-1],
                'macro_f1@100': macro_f1[99], 'macro_f1@200': macro_f1[199], 'macro_f1@300': macro_f1[299], 'macro_f1': macro_f1[-1]}
